...
using SmeKernelOperation = void (*) (
    BLASLONG, BLASLONG, BLASLONG,
    void *, BLASLONG, float,
    void *, BLASLONG,
    int8_t *, BLASLONG, void *);
...
template <SmeKernelOperation Kernel>
...
#pragma omp barrier
            for (is = mFrom; is < mTo; is += minI) {
                ...
                for(int jj = start_off; jj < js + minJ && jj < nTo; jj+= minJJ) {
                    ...
                    Kernel(minI, minJJ, minL, bufaa, lda, alpha[0],
...
)
{
    ...
    /*
     * P2 dispatches exactly once per public GEMM call.  The two template
     * instantiations preserve a direct call to their respective assembly
     * symbol inside the tile loop; do not replace this with a function-pointer
     * call, which would add an indirect branch to every 128x32 kernel call.
     */
    if (scaling.modulus == 0) {
        #pragma omp parallel for num_threads(nthreads) schedule(static) shared(sa, sb)
        for (int i = 0; i < nthreads; i++) {
            int thread_id = omp_get_thread_num();
            SmeGemmDriver<int8_sme_gemm_kernel_nn_lowbyte>(
                &newArgs, sa + (LEVEL3_GEMM_Q * LEVEL3_GEMM_P * thread_id), sb,
                mask, rangeM, rangeN, c8, ldc8, scaling);
        }
    } else {
        #pragma omp parallel for num_threads(nthreads) schedule(static) shared(sa, sb)
        for (int i = 0; i < nthreads; i++) {
            int thread_id = omp_get_thread_num();
            SmeGemmDriver<int8_sme_gemm_kernel_nn_sdiv>(
                &newArgs, sa + (LEVEL3_GEMM_Q * LEVEL3_GEMM_P * thread_id), sb,
                mask, rangeM, rangeN, c8, ldc8, scaling);
        }
    }
